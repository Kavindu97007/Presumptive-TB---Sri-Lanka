  ...
  <div class="container-md pt-2">  
    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">GeneXpert</li>
      </ol>
    </nav>
    <?php 
    function getName(){
      if ($_GET['n']==1) {
        return 'GeneXpert_date';
      }
    }
    ...
    $sql = "SELECT `{$dte}`, `{$rlst}` FROM `analytics` WHERE `ptId`=".$_GET['ptId'];
    //echo $sql;
    ...
    <form method="POST" action ="phpfiles/sputumResult.php">
      ...
      <input type="text" readonly class="form-control-plaintext" id="staticEmail" hidden value="<?php echo getName(); ?>" name="dateName" >
      <input type="text" readonly class="form-control-plaintext" id="staticEmail" hidden value="<?php echo getNameRslt(); ?>" name="rsltName" >
                ...
                <div class="col-sm">                  
                 <input type="text" id="inputRslt" class="form-control" aria-describedby="passwordHelpInline" name="rslt">
                  </div>
                 </div>
