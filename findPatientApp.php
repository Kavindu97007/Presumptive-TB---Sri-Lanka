...
<form class="row g-3" method="POST" action="findPatientApp.php">
...
$sql = "SELECT  `createdTime`,`patientId`,`givenId`,`fName`, `lName`, `gender`,  `dob`, `adrs`, `distric`, `mobileNumber`, `homeNumber` FROM `patient` WHERE ". $_POST['searchType']. " LIKE '{$fld}'";
        ...
        if ($result) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<td scope='col'><a href='appointment.php?patientId=".$row['patientId']."'>".$row['fName']." ".$row['lName']."</a></td>";
                echo "<td scope='col'>".$row['gender']."</td>";
                echo "<td scope='col'>".calculateAge($row['dob'])."</td>";
                /*echo "<td scope='col'>".$row['addressText']."</td>"*/;
                echo "<td scope='col'>".$row['adrs']."</td>";
